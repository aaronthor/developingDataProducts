#setwd("/z/coursera/9-developingDataProducts/project/")
...
# Read data
data <- fread("./data/sets.csv", encoding='Latin-1')
head(data)
...
sum(is.na(data)) # 0
length(unique(data$setId)) # 10459
...
#' @return data.table
#'
groupByYearPiece <- function(dt, minYear, maxYear, minPiece, maxPiece, themes) 
{
...
#' 
groupByTheme <- function(dt, minYear, maxYear, minPiece, maxPiece, themes) 
{
    dt <- groupByYearPiece(dt, minYear, maxYear, minPiece, maxPiece, themes) 
...
#'
groupByYearAgg <- function(dt, minYear, maxYear, minPiece, maxPiece, themes) 
{
    dt <- groupByYearPiece(dt, minYear, maxYear, minPiece, maxPiece, themes)
    result <- dt %>% 
            group_by(year)  %>% 
...
#' Aggregate dataset by year to get average number of pieces / yr
#' 
#' @param dt data.table
#' @param minYear
#' @param maxYear
...
#' @return data.table 2 columns
#'
groupByPieceAvg <- function(dt,  minYear, maxYear, minPiece, maxPiece, themes) 
{
    dt <- groupByYearPiece(dt, minYear, maxYear, minPiece, maxPiece, themes)
    result <- dt %>% 
            group_by(year) %>% 
...
#' @return data.table 2 columns
#'
groupByPieceThemeAvg <- function(dt,  minYear, maxYear, minPiece, maxPiece, themes) 
{
    dt <- groupByYearPiece(dt, minYear, maxYear, minPiece, maxPiece, themes)
    result <- dt %>% 
            group_by(theme) %>%
                             ...
                             yAxisLabel = "Average # of Pieces") 
{
    ...
    piecesByYearAvg$chart(color = c('red', 'blue', 'green'))
                                 ...
                                 yAxisLabel = "Average # of Pieces") 
{
    ...
    piecesByThemeAvg$chart(color = c('orange', 'blue', 'green'))
