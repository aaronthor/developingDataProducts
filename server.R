...
shinyServer(
  function(input, output) {
    ...
    output$address <- renderText({
        input$goButtonAdd
        isolate(paste("http://brickset.com/sets/", input$setid, sep=""))
    ...
    output$inc <- renderUI({ 
        ...
        isolate(openPage(paste("http://brickset.com/sets/", input$setid, sep="")))
    })
    ...
    dataTableByPieceMin <- reactive({
        groupByPieceMin(data, input$timeline[1], 
                        input$timeline[2], input$pieces[1],
                        input$pieces[2], input$themes)
    })
    
    dataTableByPieceMax <- reactive({
        groupByPieceMax(data, input$timeline[1], 
                        input$timeline[2], input$pieces[1],
                        input$pieces[2], input$themes)
    })
    ...
    dataTableByPieceThemeMax <- reactive({
        groupByPieceThemeMax(data, input$timeline[1], 
                             input$timeline[2], input$pieces[1],
                             input$pieces[2], input$themes)
    })
    
    dataTableByPieceThemeMin <- reactive({
        groupByPieceThemeMin(data, input$timeline[1], 
                             input$timeline[2], input$pieces[1],
                             input$pieces[2], input$themes)
    })
